 ...
 */
class Engine_Service_Paynow extends Zend_Service_Abstract {

    /**
     * Here you can define the API keys of your new payment gateway which will use later in this file. i.e. The integration key for Paynow gateway.
     *
     * @var string
     */
    protected $_integration_key;

    /**
     * Here you can define the API keys of your new payment gateway which will use later in this file. i.e. The Integration id for Paynow gateway.
     *
     * @var string
     */
    ...
    /**
     * Set credentials for your gateway. i.e. here we have set the integration key and integration id for paynow gateway. 
     */
    public function setOptions(array $options) {
        ...
        // Check options
        if (empty($this->_integration_key) || empty($this->_integration_id)) {
            throw new Engine_Service_Paynow_Exception('Not all connection ' .
    ...
    protected function _checkParams(array $params, $requiredParams = null, $supportedParams = null) {
        ...
        if (!is_array($params)) {
            if (!empty($params)) {
                throw new Engine_Service_Paynow_Exception('Invalid data type', 'UNKNOWN_PARAM');
            } else {
                $params = array();
            }
        }
        ...
        if (!empty($params)) {
            ...
            throw new Engine_Service_Paynow_Exception(sprintf('Unknown param(s): ' .
        ...
        if (count($missingRequiredParams = array_diff_key($requiredParams, $foundKeys)) > 0) {
            ...
            throw new Engine_Service_Paynow_Exception(sprintf('Missing required ' .
