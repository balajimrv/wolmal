 ...
 */
class Engine_Service_Payumoney extends Zend_Service_Abstract {
    
  
    /**
     * The Merchant key of user
     *
     * @var string
     */
    ...
    /**
     * Set credentials for your gateway. i.e. here we have set the Merchant key and salt for Payumoney gateway. 
     */
    public function setOptions(array $options) {
        ...
        // Check options
        if (empty($this->_merchant_key) || empty($this->_salt)) 
        {
            
            throw new Engine_Service_Payumoney_Exception('Not all connection '.
    ...
    // Get uri
      ...
      if( $this->_testMode ) {
        ...
        $uri = 'https://secure.payu.in/_payment';
      }
    ...
    */
    ...
    {
        if (empty($this->_merchant_key) ) {
            throw new Engine_Service_Payumoney_Exception('Not all connection ' .
            'options were specified.', 'MISSING_LOGIN');
            throw new Zend_Service_Exception('Not all connection options were specified.');
        }

        return $this->_merchant_key;
    } 
     /** give merchant salt
    * @return String
    */
 
    protected function getMerchantSalt()
    {
        if (empty($this->_salt) ) {
            throw new Engine_Service_Payumoney_Exception('Not all connection ' .
            'options were specified.', 'MISSING_LOGIN');
    ...
    protected function _checkParams(array $params, $requiredParams = null, $supportedParams = null) {
        ...
        if (!is_array($params)) {
            if (!empty($params)) {
                throw new Engine_Service_Payumoney_Exception('Invalid data type', 'UNKNOWN_PARAM');
            } else {
                $params = array();
            }
        }
        ...
        if (!empty($params)) {
            ...
            throw new Engine_Service_Payumoney_Exception(sprintf('Unknown param(s): ' .
        ...
        if (count($missingRequiredParams = array_diff_key($requiredParams, $foundKeys)) > 0) {
            ...
            throw new Engine_Service_Payumoney_Exception(sprintf('Missing required ' .
