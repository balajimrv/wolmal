...
<?php $this->headScript()
        ->appendFile($this->layout()->staticBaseUrl . 'application/modules/Timeline/externals/scripts/core.js')
...
<?php if( !$this->getUpdate && $this->feedOnly === false) : ?>

<script type="text/javascript">
    window.addEvent("load", function() {
        
        try {
            var tabs_feed = $$('li.tab_layout_timeline_feed').getLast();
            if (typeOf (tabs_feed) == 'element') {
                if (!tabs_feed.hasClass('active')) {
                    tabs_feed.addEvent('click', function(){
                        start_side();
                    });
                }
                else {
...
<ul class='feed timeline' id="activity-feed" style="opacity: 0">
<?php endif ?>
...
<?php
  foreach( $actions as $action ): // (goes to the end of the file)
    try { // prevents a bad feed item from destroying the entire page
      ...
      ob_start();
    ?>
    <?php $tmp_features = (empty ($this->features)) ? null : $this->features->getRowMatching('action_id', $action->action_id); ?>
    <?php if( !$this->noList ): ?>
        <li id="activity-item-<?php echo $action->action_id ?>" <?php if ($tmp_features !== null):?>data-side="featured"<?php endif;?>>
        <span class="scale_point"></span>
    <?php endif; ?>
<div class="tl_boxes">
          <?php  $if_delete_button = $this->viewer()->getIdentity() && (
                 ...
                 $show_div = $if_delete_button || $this->show_feature_toggle;
          ?>
        <?php if($show_div): ?>
          <div class="activity-item-nav">
        <?php endif; ?>
          <?php if($if_delete_button): ?>
              ...
              ), $this->translate(''), array('class' => 'smoothbox delete_event')) ?>
         
            <?php endif; ?>

            <?php if ($this->show_feature_toggle ):?>
                <a href="javascript:void(0);" onclick="javascript:activityUpdateHandler.toggleFeatured(<?php echo $action->action_id ?>)" class="<?php if ($tmp_features !== null):?>tl_featured<?php else:?>tl_default<?php endif;?>" id="feature_toggle_<?php echo $action->action_id ?>"></a>
            <?php endif;?>
    <?php if($show_div): ?>
        </div>
    <?php endif; ?>
...
</div><!--<div class="feed-update_ul"> end -->
<?php endif ?>
