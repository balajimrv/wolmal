...
CwCrop = new Class({
	...
	initialize: function(options)
	{
		...
		if (!window.$(this.options.cropframe) || !window.$(this.options.imgframe)) {
			return;
		}

		// calc initial limits and scaling
		this.elemsize = window.$(this.options.imgframe).getSize();
		if (this.elemsize.x == 0) {
			this.elemsize = window.$(this.options.imgframe).getOffsetParent().getSize();
		}
		...
		if (this.options.initialmax) {
			window.$(this.options.cropframe).setStyle("top", 0);
			window.$(this.options.cropframe).setStyle("left", 0);
			window.$(this.options.cropframe).setStyle("width", this.limits.x[1].toInt());
			window.$(this.options.cropframe).setStyle("height", this.limits.y[1].toInt());
		}
		else {
			window.$(this.options.cropframe).setStyle("top", this.options.initialposition.y);
			window.$(this.options.cropframe).setStyle("left", this.options.initialposition.x);
			window.$(this.options.cropframe).setStyle("width", this.limits.x[0].toInt());
			window.$(this.options.cropframe).setStyle("height", this.limits.y[0].toInt());
		}
		if (this.options.fixedratio) {
                    if (this.limits.x[1].toInt() > this.limits.y[1].toInt()) {
                        var tmp_width = window.$(this.options.cropframe).getStyle("height").toInt() * this.options.fixedratio;
                        if (tmp_width > this.limits.x[1].toInt()) {
                            var tmp_height = window.$(this.options.cropframe).getStyle("width").toInt() / this.options.fixedratio;
                            window.$(this.options.cropframe).setStyle("height", tmp_height.toInt());
                        }
                        else {
                            window.$(this.options.cropframe).setStyle("width", tmp_width);
                        }
                    }
                    else {
                        var tmp_height = window.$(this.options.cropframe).getStyle("width").toInt() / this.options.fixedratio;
		...
		this.updateCropDims(window.$(this.options.cropframe));
		this.recalcResize(window.$(this.options.cropframe));

		this.myMove = new Drag.Move(window.$(this.options.cropframe), {
			style: true,
			precalculate: true,
			snap: 1,
			grid: false,
			container: window.$(this.options.imgframe),
			includeMargins: true,
			checkDroppables: false,
			handle: window.$(this.options.draghandle),
		...
		this.moveBgImage(window.$(this.options.cropframe));

		this.myResizeXY = window.$(this.options.cropframe).makeResizable({
			style: true,
			precalculate: true,
			snap: 1,
			grid: false,
			handle: window.$(this.options.resizehandle),
			limit: this.limits,
		...
		if (window.$(this.options.cropbtn)) {
			window.$(this.options.cropbtn).addEvent('click', function() {
				this.doCrop();
			}.bind(this));
		}
	},
	...
	{
		if (!window.$(this.options.cropdims)) {
			return;
		}
		if (displayPosition) {
		...
		window.$(this.options.cropdims).set('html', out);
	},

	doCrop: function()
	{
		el = window.$(this.options.cropframe);
