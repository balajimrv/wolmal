...
class Sesadvancedactivity_Form_Admin_Settings_General extends Engine_Form {
  ...
  public function init() {
    ...
    if( !$settings->getSetting('sesbasic.defaultcurrency')) {
      $this->addElement('Select', 'sesbasic_defaultcurrency', array(
        'label' => 'Default Currency',
        'description' => 'Choose the default currency for the Event Tickets on your website. (Note: This is a one time setting, so you would not be able to change that later. If you wish to change the default currency in future, then please contact our support team.)',
        'multiOptions' => Engine_Api::_()->sesbasic()->getSupportedCurrency(),
        'value' => $settings->getSetting('sesbasic.defaultcurrency','USD'),
      ));
    }
...
//         ));
        ...
        $description = "<div class='tip'><span>" . Zend_Registry::get('Zend_Translate')->_('You have not installed our "People You May Know Plugin". So, you can purchase from our site.') . "</span></div>";
        //Add Element: Dummy
        $this->addElement('Dummy', 'sesadvancedactivity_pymkenable', array(
            'label' => "Enable 'People You May Know' In Feeds",
            'description' => 'This setting is dependent on our “People You May Know Plugin". So, it will work only if you have this plugin installed and enabled on your website.',
        ));  
        $this->sesadvancedactivity_pymkenable->addDecorator('Description', array('placement' => Zend_Form_Decorator_Abstract::PREPEND, 'escape' => false));
      }
      
      // Assign the composing values
      $composePartials = array('tagUseses'=>'Tag People (Members will be able to tag their friends to their status posts.)','smilesses'=>'Emoticons (Members will be able to add smileys and emoticons to their posts. Our Advanced Nested Comments with Attachments Plugin)','locationses'=>'Check In (Members will be able to add their location to their posts)','shedulepost'=>'Schedule Post (Members will be able to choose publish date and time for their posts.)');
      ...
      $this->addElement('MultiCheckbox', 'sesadvancedactivity_statusboxsettings', array(
        'label' => "Status Box Options Settings (You can also re-order the attachment from below setting to change their sequence.)",
      ));
      
      $this->addElement('MultiCheckbox', 'sesadvancedactivity_composeroptions', array(
        'label' => 'Status Box Attachments',
        'description' => 'Select the attachments which will be available in status box. To reorder the attachments, click on their row and drag them up or down and click "Save Changes" button to save your changes.',
    ...
    $this->sesadvancedactivity_whatsnewicon->addDecorator('Description', array('placement' => Zend_Form_Decorator_Abstract::PREPEND, 'escape' => false));
      ...
      $this->addElement('Select', 'sesadvancedactivity_allownetworkprivacy', array(
        'label' => 'Allow To Choose Network Privacy',
        'description' => 'Do you want to allow members to choose network privacy for their status updates?',
        'multiOptions' => array(
            1 => 'Yes',
            0 => 'No'
        ),
        'onchange'=>'hideShowNetworkPrivacySettings(this.value)',
        'value' => $settings->getSetting('sesadvancedactivity.allownetworkprivacy', 1),
      ));
      
      $this->addElement('Select', 'sesadvancedactivity_allownetworkprivacytype', array(
        'label' => 'Network Privacy Options',
        'description' => 'Choose from below the network privacy options that will be available to members.',
        'multiOptions' => array(
            1 => 'Members Joined Networks',
            0 => 'All Networks of Website'
        ),
        'value' => $settings->getSetting('sesadvancedactivity.allownetworkprivacytype', 0),
      ));
      ...
      //Instragram Settings
//       $this->addElement('Radio', 'sesadvancedactivity_instagram_enable', array(
//         'label' => 'Enable Instagram',
//         'description' => 'Do you want to enable instagram ?',
//         'multiOptions' => array(
//             1 => 'Yes',
//             0 => 'No'
//         ),
//         'onclick' => 'showinstagram(this.value);',
//         'value' => $settings->getSetting('sesadvancedactivity.instagram.enable', 0),
//       ));
//       
...
//       ));
//       
//       $this->addElement('Text', "sesadvancedactivity_photoshowcount", array(
//         'label' => 'Photo Couts',
//         'validators' => array(
//             array('Int', true),
//             array('GreaterThan', true, array(0)),
//         ),
//         'value' => $settings->getSetting('sesadvancedactivity.photoshowcount',8),
//       ));
//       
//       $this->addElement('Text', 'sesadvancedactivity_instagramtext', array(
//         'label' => "Instagram Tab Title",
//         'description' => 'Enter the title of the Instagram tab. If you do not want to show title, then simply leave this field empty.',
//         'value' => $settings->getSetting('sesadvancedactivity.instagramtext', 'Instagram'),
//       ));
//           
//       if (count($default_photos_main) > 0) {
//         $default_photos = $default_photos_main;
//         $this->addElement('Select', 'sesadvancedactivity_instagramicon', array(
//           'label' => 'Instagram Tab Icon',
//           'description' => 'Choose icon of the Instagram tab. [Note: You can add a new icon from the "File & Media Manager" section from here: <a target="_blank" href="' . $fileLink . '">File & Media Manager</a>. Leave the field blank if you do not want to show icon.]',
//           'multiOptions' => $default_photos,
//           'value' => $settings->getSetting('sesadvancedactivity.instagramicon', ''),
//         ));
//       } else {
//         $description = "<div class='tip'><span>" . Zend_Registry::get('Zend_Translate')->_('There are currently no icon for Instagram Tab. Icon to be chosen for Instagram Tab should be first uploaded from the "Layout" >> "<a target="_blank" href="' . $fileLink . '">File & Media Manager</a>" section. => There are currently no Icon in the File & Media Manager for Instagram Tab. Please upload the Icon to be chosen for Instagram Tab from the "Layout" >> "<a target="_blank" href="' . $fileLink . '">File & Media Manager</a>" section.') . "</span></div>";
//         //Add Element: Dummy
//         $this->addElement('Dummy', 'sesadvancedactivity_instagramicon', array(
//             'label' => 'Instagram Tab Icon',
//             'description' => $description,
//         ));
//       }
//       $this->sesadvancedactivity_instagramicon->addDecorator('Description', array('placement' => Zend_Form_Decorator_Abstract::PREPEND, 'escape' => false));
