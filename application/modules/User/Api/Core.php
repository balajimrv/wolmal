...
class User_Api_Core extends Core_Api_Abstract
{
  ...
  public function getInviter($user_id){
    $db = Engine_Db_Table::getDefaultAdapter();
    $result = $db->query('SELECT a.displayname, a.user_id 
    FROM engine4_users AS a
    LEFT JOIN engine4_inviter_invites AS b 
    ON b.user_id = a.user_id 
    WHERE b.new_user_id = "'.$user_id.'"')->fetch();
    return $result;
  }
}
