...
class Sesbasic_AdminLightboxController extends Core_Controller_Action_Admin {
    ...
    $form->populate($permissionsTable->getAllowed('sesbasic_member_level', $id, array_keys($form->getValues())));
    // Check post
    ...
    try {
      // Set permissions
      $permissionsTable->setAllowed('sesbasic_member_level', $id, $values);
      // Commit
      $db->commit();
    } catch (Exception $e) {
      $db->rollBack();
