...
<table class='admin_table'>
  <thead>
    <tr>
      ...
      <th><?php echo $this->translate("100016195") ?></th>
  ...
  <tbody>
    <?php foreach ($this->paginator as $item): ?>
      <tr>
        <td><input type='checkbox' class='checkbox' value="<?php echo $item->userpointearner_id ?>"/></td>
        <td><?php echo Engine_String::strlen($item->userpointearner_title) > 50 ? Engine_String::substr($item->userpointearner_title, 0, 50) . '...' : $item->userpointearner_title; ?></td>
        <td><?php echo $item->userpointearnertype_typename ?></td>
        <td><?php echo $item->userpointearner_cost ?></td>
        <td><?php echo $item->userpointearner_engagements ?></td>
        <td><?php echo $this->locale()->toNumber($item->userpointearner_views) ?></td>
        <td><?php if($item->userpointearner_enabled): ?><?php echo $this->translate('Yes'); ?><?php else : ?><?php echo $this->translate('No'); ?><?php endif; ?></td>
        <td><?php echo $this->timestamp($item->userpointearner_date) ?></td>
        
        <td nowrap='nowrap'>
          <a href='<?php echo $this->url(array('action' => 'edit', 'item_id' => $item->userpointearner_id));?>'><?php echo $this->translate('100016176'); ?></a> | <a href='javascript:enable_offer(<?php echo $item->userpointearner_id ?>, <?php echo $item->userpointearner_enabled ? '0' : '1' ?>)'>
          <?php if(!$item->userpointearner_enabled): ?><?php echo $this->translate('100016187'); ?><?php else: ?><?php echo $this->translate('100016177'); ?><?php endif; ?></a> | <a href='javascript:delete_offer(<?php echo $item->userpointearner_id ?>)'><?php echo $this->translate('100016180'); ?></a>
        </td>
...
<td>
  ...
  <div id="upselector_div" style="display:none">
    <form action='<?php echo $this->url(array('action'=>'edit'));?>' method='get' name='items'>
      ...
      <?php foreach($this->offer_types as $offer_type): ?><option value='<?php echo $offer_type['userpointearnertype_id']; ?>'><?php echo $offer_type['userpointearnertype_title'] ?></option><?php endforeach; ?></select>&nbsp;
