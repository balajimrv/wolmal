...
class Sitegateway_Installer extends Engine_Package_Installer_Module {
    ...
    public function onPreInstall() {
        ...
        $PRODUCT_DESCRIPTION = 'Advanced Payment Gateways / Stripe Connect /  PayPal Adaptive ';
        $PRODUCT_TITLE = 'Advanced Payment Gateways / Stripe Connect /  PayPal Adaptive ';
    ...
    public function onInstall() {
        ...
        if (!($this->isGatewayExist('Sitegateway_Plugin_Gateway_PayPalAdaptive'))) {
            $db->query("INSERT IGNORE INTO `engine4_payment_gateways` (`title`, `description`, `enabled`, `plugin`, `test_mode`) VALUES ('PayPalAdaptive', NULL, 0, 'Sitegateway_Plugin_Gateway_PayPalAdaptive', 0);");
        }
        ...
        $db->query("UPDATE `engine4_core_modules` SET `title` = 'Advanced Payment Gateways / Stripe Connect /  PayPal Adaptive', `description` ='Advanced Payment Gateways / Stripe Connect /  PayPal Adaptive' where name='sitegateway';");
        $db->query("UPDATE `engine4_core_menuitems` SET `label` = 'SEAO - Advanced Payment Gateways / Stripe Connect /  PayPal Adaptive' where name='core_admin_main_plugins_sitegateway' and module='sitegateway';");
