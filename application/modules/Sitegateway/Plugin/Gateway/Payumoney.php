...
//customer email
    ...
    public function onUserOrderTransactionReturn(Payment_Model_Order $order, array $params = array()) {
        ...
        if ($paymentStatus == 'okay') {
          ...
          //echo "<prE>";var_dump($user_order->toArray());die;
          $user_order->discount_value = $discountValue;
          $user_order->save();  
          
        	//START DATA UPDATION WORK IN POINTS TABLE
        	/*$userpoints = Engine_Api::_()->getApi('core', 'activitypoints')->getPoints($order->user_id);
        	if(isset($_SESSION['cart_discount_value']))
        	$discountValue = $_SESSION['cart_discount_value'];
          ...
          $order->save();
        	...
        	Engine_Api::_()->getDbtable('points', 'activitypoints')->update(array('userpoints_count' => $remainingPoints, 'userpoints_totalspent' => $spentPoints), array('userpoints_user_id =?' => $order->user_id));
        	//END DATA UPDATION WORK IN POINTS TABLE
        	$divided_amount = 1000;
        	$sub_total = $user_order->sub_total;
            ...
            $firstParentId = $inviterTable->select()->from($inviterTable->info('name'), 'user_id')->where('new_user_id = ?', $order->user_id)->query()->fetchColumn();
              if($firstParentId) {
