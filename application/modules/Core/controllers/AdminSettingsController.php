...
//        }
  ...
  public function performanceAction()
  {
    ...
    foreach( array_keys($form->getElement('type')->options) as $backend ) {
      ...
      if( 'Engine_Cache_Backend_Apc' == $backend && !extension_loaded('apcu') ) {
        $disabledTypeOptions[] = $backend;
      }
    ...
    if( $this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost()) ) {
      ...
      if( !$currentCache['frontend']['core']['caching'] ) {
        ...
        if( $currentCache['frontend']['core']['caching'] && @$backend->save('test_value', 'test_id') && @$backend->test('test_id') ) {
