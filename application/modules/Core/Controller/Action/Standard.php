...
abstract class Core_Controller_Action_Standard extends Engine_Controller_Action
{
  ...
  protected function exceptionWrapper(Exception $exception, Engine_Form $form = null, Zend_Db_Adapter_Abstract $db = null)
  {
    if ($db !== null) {
      $db->rollBack();
    }

    if (!empty($_FILES['Filedata']) && (isset($_SERVER['HTTP_USER_AGENT']) && preg_match('/adobe/i', $_SERVER['HTTP_USER_AGENT'])) ) {
      header('Content-Type: application/json');
      echo Zend_Json::encode(array('error' => $exception->getMessage()));
      exit;
    }
