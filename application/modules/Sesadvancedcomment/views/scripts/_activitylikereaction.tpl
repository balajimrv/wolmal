...
<?php if(!$this->isOnThisDayPage){ ?>
  <div class="comments_stats_comments comment_stats_<?php echo $action->getIdentity(); ?>">
    <?php if($commentCount > 0){ ?>
      <?php echo $this->partial('_commentstats.tpl','sesadvancedcomment',array('action'=>$action,'commentCount'=>$commentCount));  ?>
    <?php } ?>
  </div>
<?php } ?>                
</li>
