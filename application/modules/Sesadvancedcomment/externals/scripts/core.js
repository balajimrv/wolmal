...
function sesadvancedcommentlike(action_id, comment_id,obj,page_id,type,sbjecttype,subjectid) {
...
function sesadvancedcommentunlike(action_id, comment_id,obj,page_id,type,sbjecttype,subjectid) {
  var ajax = new Request.JSON({
...
sesJqueryObject(document).on('click','.sesadvancedcomment_edit',function(e){
  ...
  if(filesLength.length){
    for(var i =0; i<filesLength.length;i++){
     ...
     image = '<img src="'+filesrc+'"><a href="javascript:;" data-url="'+sesJqueryObject(filesLength[i]).attr('data-fileid')+'" class="cancel_upload_file fa fa-close" title="Cancel"></a>'+videoBtn;
     display = 'block';
  ...
  var d = new Date();
  var time = d.getTime();
  var html = '<div class="comment_edit _form_container sesbasic_clearfix"><form class="sesadvancedactivity-comment-form-edit" method="post"><div class="comment_form sesbasic_clearfix"><textarea class="body" name="body" id="'+time+'" cols="45" rows="1" placeholder="Write a comment...">'+textBody+'</textarea><div class="_sesadvcmt_post_icons sesbasic_clearfix"><span>'+imageLink+'<input type="file" name="Filedata" class="select_file" multiple style="display:none;">'+module+subjectInputs+'<input type="hidden" name="file_id" class="file_id" value="'+fileid+'"><input type="hidden" class="file" name="comment_id" value="'+comment_id+'"></span>'+videoLink+'<span><a href="javascript:;" class="emoji_comment_select"></a></span></div></div><div class="uploaded_file"  style="display:none;"></div><div class="upload_file_cnt">'+files+'</div><div class="sesadvcmt_btns" style="margin-top:0px;"><a href="javascript:;" class="sesadvancedcomment_cancel">cancel</a></div></form></div>';
    ...
    'onComplete' : function(responseHTML) {
      if(typeof responseHTML.status != 'undefined' && responseHTML.status){
         ...
         if(!form.find('.upload_file_cnt').length){
          ...
          sesJqueryObject(uploadFile).html('<img src="'+src+'"><a href="javascript:;" data-url="'+videoid+'" class="cancel_upload_file fa fa-close" title="Cancel"></a><a href="javascript:;" class="sescmt_play_btn fa fa-play"></a>');
          complitionRequestTrigger();
          Smoothbox.close();
      }else{
         sesJqueryObject('#sesadvvideo_txt').css('border','1px solid red');
      ...
      success:function(data){
        ...
        try{
          ...
          if(dataJson.status == 1){
            ...
            sesJqueryObject(uploadFile).html('<img src="'+dataJson.src+'"><a href="javascript:;" data-url="'+dataJson.photo_id+'" class="cancel_upload_file fa fa-close" title="Cancel"></a>');
            complitionRequestTrigger();
              //silence
          }else{
            //sesJqueryObject(form).find('.file_id').val('');
            //sesJqueryObject(form).find('.uploaded_file').hide();
            sesJqueryObject(uploadFile).append('<a href="javascript:;" class="cancel_upload_file fa fa-close" title="Cancel"></a>');	
          }
        }catch(err){
          sesJqueryObject(uploadFile).append('<a href="javascript:;" class="cancel_upload_file fa fa-close" title="Cancel"></a>');	
          //silence
...
sesJqueryObject(document).click(function(e){
  if(sesJqueryObject(e.target).hasClass('emoji_comment_select')  || sesJqueryObject(e.target).attr('id') == 'sesadvancedactivityemoji-edit-a' || sesJqueryObject(e.target).attr('id') == "emotions_target")
    return;
...
sesJqueryObject(document).on('click','a.emoji_comment_select',function(){
  ...
  if(sesJqueryObject(this).hasClass('activity_emoji_content_a') && typeof sesadvancedactivityDesign != 'undefined' && sesadvancedactivityDesign == 2){
    var leftSub = 55;  
  }else
    var leftSub = 264;
...
function activityFeedAttachment(that){
  ...
  sesJqueryObject('#compose-tray').html('<div class="sesact_composer_sticker"><img src="'+image+'"><a class="remove_reaction_image_feed notclose fa fa-close" href="javascript:;"></a></div>');
...
function showhidecommentsreply(comment_id, action_id) {
  if($('comments_reply_'+comment_id+'_'+action_id).style.display == 'block') {
    ...
    if(sesJqueryObject('#hideshow_'+comment_id+'_'+action_id))
      sesJqueryObject('#hideshow_'+comment_id+'_'+action_id).removeClass('fa fa-minus').addClass('fa fa-plus-square-o');
  } else {
    ...
    if(sesJqueryObject('#hideshow_'+comment_id+'_'+action_id))
      sesJqueryObject('#hideshow_'+comment_id+'_'+action_id).removeClass('fa fa-plus-square-o').addClass('fa fa-minus');
  }
}