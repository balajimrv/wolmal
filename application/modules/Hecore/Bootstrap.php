...
if (!function_exists('console_log')) {
  function console_log($var, $return = false, $ip = null)
  {
    if (($ip && $_SERVER['REMOTE_ADDR'] == $ip) || !$ip) {
      $bt = debug_backtrace();
      $caller = array_shift($bt);
...
if (!function_exists('print_session')) {
    function print_session($var, $key = null)
    {
        if(!is_null($key) && isset($_GET[$key])){
            HELOG::reg($key);
            //print_die("session created :".$key);
        }
...
//experement by Kalyskin
if (!class_exists('HELOG')) {
  class HELOG {
      public static function d($var,$key){
          if(isset($_GET[$key])){
              self::reg($key);
          }
          if($_SESSION[$key]){
              $return = false;
              $ip = null;
              print_die($var,$return,$ip);
          }
      }
      public static function arr($var ,$key ){
          if(isset($_GET[$key])){
              self::reg($key);
          }
          if($_SESSION[$key]){
